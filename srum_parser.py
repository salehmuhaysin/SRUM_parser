...
def main():
	# ================== arguments
	a_parser = argparse.ArgumentParser('Python script tool to parse Windows System Resource Usage Monitor (SRUM)' , description='SRUM TYPES: NetworkDataUsageMonitor, NetworkConnectivityUsageMonitor, ApplicationResourceUsage')
	
	requiredargs = a_parser.add_argument_group('required arguments')
	requiredargs.add_argument('-i', dest='in_file', help='Input path for SRUDB.dat file', required=True)
	requiredargs.add_argument('-o' , dest='out_file' , help='Output path (srum.csv) will output folder as srum-[srum_type].csv', required=True)
	
	a_parser.add_argument('-r' , dest='result_type' , help='Result files (json, csv), by default output in csv file')

	args = a_parser.parse_args()

	# =================== Parsing
	if not os.path.isfile(args.in_file):
		print "[-] Input file ["+args.in_file+"] not exists"
	srum = SRUM_Parser( args.in_file )

	# =================== output 
	result_type = 'csv'
	if args.result_type is not None and args.result_type == 'json':
		result_type = 'json'

	srum.write_output_main( args.out_file , result_type)
